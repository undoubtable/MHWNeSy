# -*- coding: utf-8 -*-
"""
Apply event-level verifier to correct U-Net MHW prediction masks.

Idea:
    U-Net pred_mask -> connected components -> event verifier score
    If event score < threshold, remove this predicted component.
...
def correct_split(split, threshold, data_dir, pred_dir, event_dir, verifier_dir, out_dir, save_mask=True):
...
def main():
    ...
    parser.add_argument("--thresholds", type=str, default="0.30,0.40,0.50,0.60,0.70")
    ...
    parser.add_argument(
        "--verifier_dir",
        type=str,
        default=str(cfg.EVENT_DIR / "08_figure_event_verifier_cnn"),
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default=str(cfg.UNET_RUN_DIR / "09_unet_plus_figure_verifier"),
    )
