# -*- coding: utf-8 -*-
"""
Make event-level candidate patches from the multichannel U-Net forecast.

Inputs:
    outputs/03b_forecast_dataset_multichannel_h10_l5/
        X_{split}.npy, y_{split}.npy
    outputs/04c_unet_multichannel_h10_l5/
        pred_prob_{split}.npy, selected_threshold.json

Outputs:
    outputs/04c_unet_multichannel_h10_l5/
        pred_mask_{split}.npy
    outputs/06c_neurrl_event_dataset_from_multichannel_h10_l5/
        figure_event_{split}.npz

Each .npz contains:
    X           float16 [N, 41, crop, crop]
                first 40 channels: multichannel forecast inputs
                last channel: predicted candidate component mask
...
def write_pred_mask(prob_path, mask_path, threshold, chunk_size):
    prob = np.load(prob_path, mmap_mode="r")
    N, H, W = prob.shape
    mask = np.lib.format.open_memmap(mask_path, mode="w+", dtype=np.uint8, shape=(N, H, W))

    for start in tqdm(range(0, N, chunk_size), desc=f"Mask {prob_path.name}"):
        end = min(start + chunk_size, N)
        mask[start:end] = (np.array(prob[start:end], dtype=np.float32) >= threshold).astype(np.uint8)

    mask.flush()
    return np.load(mask_path, mmap_mode="r")


def process_split(args, split, threshold, channel_names):
    ...
    mask_path = pred_dir / f"pred_mask_{split}.npy"
    pred_all = write_pred_mask(prob_path, mask_path, threshold, args.mask_chunk_size)

    N, C, H, W = X_all.shape
    print(f"[{split}] X={X_all.shape}, y={y_all.shape}, pred={pred_all.shape}, threshold={threshold:.4f}")
    if len(channel_names) != C:
        channel_names = [f"input_ch_{j:03d}" for j in range(C)]
    ...
    np.savez_compressed(
        ...
        source_channel_names=np.array(channel_names + ["predicted_component_mask"]),
        selected_threshold=np.array([threshold], dtype=np.float32),
        pred_mask_file=np.array([str(mask_path)]),
        description=np.array([
            "X shape [N, 41, crop, crop]. First 40 channels are multichannel forecast inputs; last channel is predicted component mask."
        ]),
    )
...
def main():
    ...
    threshold = load_selected_threshold(args.selected_threshold_json)
    meta_path = Path(args.data_dir) / "meta.json"
    channel_names = []
    if meta_path.exists():
        channel_names = json.loads(meta_path.read_text(encoding="utf-8")).get("channel_names", [])

    for split in args.splits.split(","):
        split = split.strip()
        if split:
            process_split(args, split, threshold, channel_names)
